...
class DecryptScraper:
    def __init__(self, max_concurrent: int = 5):
        ...
        self.article_base_url = (
            "https://decrypt.co/_next/data/1DAsWX9DvodIzQ0WRMepi/en-US"
        )
    ...
    async def get_full_article_content(
        self, session: aiohttp.ClientSession, article_id: str, slug: str
    ) -> str:
        try:
            ...
            async with session.get(url, params=params) as response:
                ...
                if clean_content:
                    ...
                    safe_print(
                        f"{Fore.RED}✗ No content found for: {slug}{Style.RESET_ALL}"
                    )
                    return ""
    ...
    async def format_article_with_content(
        ...
        try:
            ...
            if featuredImage:
                imageUrl = featuredImage.get("src", "")
            else:
                imageUrl = ""
            tags = [tag.get("name") for tag in article.get("tags", {}).get("data", [])]
            formatted_article = {
                "id": article_id,
                "slug": slug,
                "title": title,
                ...
                "authorName": (
                    article["authors"]["data"][0]["name"]
                    if article.get("authors", {}).get("data")
                    else None
                ),
                "category": (
                    article["category"]["data"]["name"]
                    if article.get("category", {}).get("data")
                    else None
                ),
                "sourceName": self.source_name,
                "sourceUrl": self.source_url,
                "imageUrl": imageUrl,
                "articleUrl": f"{self.source_url}/{article_id}/{slug}",
                "tags": [*tags],
            }
    ...
    async def process_articles_async(
        ...
        tasks = [
            self.format_article_with_content(session, article) for article in articles
        ]
        return await asyncio.gather(*tasks)

    async def get_articles_async(
        self, page: int = 0, page_size: int = 12
    ) -> List[Dict[str, Any]]:
        if page < 0:
            raise ValueError(
                f"{Fore.RED}Page number cannot be negative{Style.RESET_ALL}"
            )

        safe_print(f"{Fore.YELLOW}⟳ Fetching Decrypt articles list...{Style.RESET_ALL}")
        ...
        try:
            async with aiohttp.ClientSession(headers=self.headers) as session:
                ...
                processed_articles = await self.process_articles_async(
                    session, articles
                )
                safe_print(
