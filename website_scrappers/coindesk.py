...
class CoindeskScraper:
    ...
    def extract_text_from_html(self, html_content: str) -> str:
        ...
        for p in article.find_all('p'):
            ...
            if text and not any(footer in text.upper() for footer in ['ABOUT', 'CONTACT', 'POLICIES']):
    ...
    async def get_article_content(self, session: aiohttp.ClientSession, pathname: str) -> str:
        try:
            async with self.semaphore:
                ...
                async with session.get(url, headers=self.headers) as response:
                    ...
                    if content:
                        safe_print(f"{Fore.GREEN}✓ Successfully fetched content for article {pathname}{Style.RESET_ALL}")
                        return content
                    else:
                        safe_print(f"{Fore.YELLOW}⚠ No content found for article {pathname}{Style.RESET_ALL}")
                        return ""
                    
        except Exception as e:
            safe_print(f"{Fore.RED}✗ Error fetching article content for {pathname}: {e}{Style.RESET_ALL}")
            return ""

    def extract_articles_from_response(self, response_text: str) -> List[Dict[str, Any]]:
        try:
            ...
            articles_json_str = response_text[start_index + len('"articles":'):end_index + 1]
            return json.loads(articles_json_str)
        except Exception as e:
            safe_print(f"{Fore.RED}✗ Error parsing articles JSON: {e}{Style.RESET_ALL}")
            return []

    async def format_article(self, session: aiohttp.ClientSession, article: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def process_articles_async(self, session: aiohttp.ClientSession, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        tasks = [
            self.format_article(session, article)
